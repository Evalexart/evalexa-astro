---
title: Evalexa — Legal notice
description: Legal notice for the Evalexa project.
eyebrow: Legal documents
heading: Legal notice
intro: Information applicable to the evalexa.fr domain and its subdomains.
legalSections:
- title: Publishing
  content: 'The **evalexa.fr** website is published by Eva Lain, artist

    name, as an individual.'
- title: Scope
  content: 'This document applies to the **evalexa.fr** domain and all of

    its subdomains.'
- title: Contact
  content: 'For any request related to the site or the project:

    [contact@evalexa.fr](mailto:contact@evalexa.fr)'
- title: Hosting and distribution
  content: 'The **evalexa.fr** domain and its subdomains are hosted and

    distributed via Cloudflare, Inc. — 101 Townsend St, San Francisco,

    CA 94107, United States — cloudflare.com.'
- title: Identification
  content: 'The publisher’s identifying information that is not published directly on

    this page is held by the hosting provider and may be disclosed to competent

    authorities within the applicable legal framework.'
---
